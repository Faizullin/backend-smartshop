...
def actual_purchase_index(request):
    actual_purchases = Purchase.objects.all()#filter(status = "PENDING")
    context = {
...
def actual_purchase_edit(request, pk):
    actual_purchase = get_object_or_404(Purchase, pk=pk)
    if request.method == 'POST':
        form = PurchaseForm(request.POST, request.FILES, instance=actual_purchase)
        if form.is_valid():
            form.save()
            return JsonResponse({'success': True})
        # else:
        ...
        form = PurchaseForm(instance=actual_purchase, user=request.user)
    return render(request, 'dashboard/tables/form_base.html', {'form': form, 'edit_url': reverse('dashboard:actual_purchase_edit', kwargs={'pk': actual_purchase.pk}) })

@login_required
@group_required(['shop-owner'])
def actual_purchase_delete(request, pk):
    actual_purchase = get_object_or_404(Purchase, pk=pk)
    if request.method == 'POST':
        actual_purchase.delete()
        return redirect('dashboard:actual_purchase_index')
    raise Http404
    ...
    actions = tables.TemplateColumn(
        '<div class="dropdown">'
            '<button class="btn btn-secondary dropdown-toggle" type="button" '
                ...
                '<a class="dropdown-item edit-button" data-url="{% url \'dashboard:actual_purchase_edit\' pk=record.pk %}">Edit</a>'
                '<a class="">'
                    '<form method="post" action="{% url \'dashboard:actual_purchase_delete\' pk=record.pk %}">'
                        '{% csrf_token %}'
...
class PurchaseListView(LoginRequiredMixin,OwnerRequiredMixin,tables.SingleTableMixin,FilterView):
    ...
    template_name = 'dashboard/tables/actual_purchase/index.html'
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        return Purchase.objects.filter(id__in = purchase_item_ids, status = "PENDING")