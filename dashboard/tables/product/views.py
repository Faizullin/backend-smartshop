...
class ImageColumn(tables.Column):
    def render(self, value):
        if not value:
            return format_html('<img src="/media/product/image/unknown-product.jpg" />', value)
        return format_html('<img src="/media/images/{value}.jpg" />', value)
    
class ProductTable(tables.Table):
    image = ImageColumn(orderable=False)
    actions = tables.TemplateColumn(
        '<div class="dropdown">'
            '<button class="btn btn-secondary dropdown-toggle" type="button" '
                ...
                '<a class="dropdown-item" href="{% url \'dashboard:product_index\' %}?shop={{ record.shop.pk }}">Products</a>'
                '<a class="">'
